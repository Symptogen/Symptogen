...
bool LevelManager::VisitExit(const tinyxml2::XMLElement& element) {
	...
	if(0 == elementValue.compare("Position")) {
		...
		EntityManager::getEntityManagerInstance()->addRenderEntity(rEntity, 0); // TODO : set the layer from XML
		if(m_currentMetaEntity.m_isPhysic) {
			// std::cerr << "create physic entity" << std::endl;
			PhysicalEntity* pEntity = new PhysicalEntity(EntityManager::getEntityManagerInstance()->getPhysicalManager()->getWorld(), b2Vec2((float32)m_currentMetaEntity.m_posX, (float32)m_currentMetaEntity.m_posY));
			EntityManager::getEntityManagerInstance()->addPhysicalEntity(pEntity);
		}
